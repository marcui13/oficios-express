...
export default async function HomePage({ searchParams }: PageProps) {
  ...
  return (
    <div className="space-y-8 sm:space-y-12">
      {/* Hero Header */}
      <section className="relative overflow-hidden bg-gradient-to-b from-blue-50 to-slate-50 border-b border-slate-200 py-8 sm:py-14">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-800 mb-3">
              <MapPin className="h-3.5 w-3.5" />
              Exclusivo Rosario, Santa Fe
            </div>
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 sm:text-4xl md:text-5xl leading-tight">
              Solucioná los arreglos de tu casa con profesionales de oficio.
            </h1>
            <p className="mt-3 text-sm sm:text-base text-slate-600 leading-relaxed">
              Elegí el oficio que necesitás, consultá los profesionales en tu zona de Rosario y contactalos directo por WhatsApp.
            </p>
          </div>

          {/* Filtros rápidos: Oficio y Zona */}
          <div className="mt-6 rounded-2xl bg-white p-3.5 sm:p-5 shadow-xs border border-slate-200">
            <form method="GET" action="/" className="grid grid-cols-1 sm:grid-cols-12 gap-3">
              <div className="sm:col-span-5">
                ...
                <select
                  name="trade"
                  defaultValue={trade || ""}
                  className="w-full h-11 appearance-none rounded-xl border border-slate-300 bg-white px-3 text-sm font-medium text-slate-800 focus:border-blue-500 focus:outline-none"
                >
              ...
              <div className="sm:col-span-2 flex items-end">
                <button
                  type="submit"
                  className="w-full h-11 inline-flex items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white shadow-xs hover:bg-blue-700 active:scale-98 transition"
                >
          ...
          {TRADES.map((t) => {
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center justify-center p-2.5 sm:p-4 rounded-xl border text-center transition-all ${
                  isSelected
              ...
              >
                <div
                  className={`flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-lg mb-1.5 sm:mb-2 ${
                    isSelected ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700"
                  }`}
                >
                  <TradeIcon name={t.id} className="h-4 w-4 sm:h-5 sm:w-5" />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPros.map((pro) => {
              ...
              return (
                <div
                  key={pro.id}
                  className="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-xs hover:border-slate-300 hover:shadow-sm transition"
                >
                  <div>
